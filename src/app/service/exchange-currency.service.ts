...
import {MappedCurrencyRateObject} from '../model/exchange-rates.model';
...
@Injectable()
export class ExchangeCurrencyService implements OnInit {
  ...
  getCurrentDate(separator: string): string {
    ...
    this.currentDate = [currentDay, currentMonth, currentYear]
      .map(ExchangeCurrencyService.toTwoDigits)
      .join(separator);
